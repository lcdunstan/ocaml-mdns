...
- [ ] Extend tcpip-stack-direct with support for UDP multicast
  ...
  - [ ] Ipv4.allocate_frame needs an option to override the default IP TTL: https://github.com/mirage/mirage-tcpip/issues/95
...
- When any request is received (includes probes)
  ...
  - If it matches a unique record, ignoring A vs AAAA (and there are no other questions in the request (6.0.10, 6.3))
    - If the record is currently probing and the received request is a probe (Authority Section) (8.2)
    ...
    - Else if probing in progress and the received request is NOT a probe
      - Do nothing ?
